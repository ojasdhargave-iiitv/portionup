...
export default function AddMealScreen() {
  ...
  // Fetch nutrition preview whenever food items change
  useEffect(() => {
    const fetchPreview = async () => {
      ...
      try {
        ...
        const res = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/user/meal/preview`, {
          method: 'POST',
          headers: {
        ...
        if (res.ok) {
          ...
          // Simple health score: ratio of protein+fiber to total macros
          const totalMacros = (data.protein || 0) + (data.carbs || 0) + (data.fat || 0) + (data.fiber || 0);
          if (totalMacros > 0) {
            const score = Math.round(((data.protein || 0) + (data.fiber || 0)) / totalMacros * 100);
